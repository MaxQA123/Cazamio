...
{
    public class DBRequestApplicationRequiredDocuments
    {
        ...
        public class ApplicationRequiredDocuments
        {
            public static string DeleteRecordByApartmentApplicationId(long? apartmentApplicationId, string emailTenant, int marketplaceId)
            {
                ...
                {
                    SqlCommand command = new("DELETE FROM ApplicationRequiredDocuments WHERE ApartmentApplicationId = ApartmentApplicationId AND TenantId" +
                               " IN" +
