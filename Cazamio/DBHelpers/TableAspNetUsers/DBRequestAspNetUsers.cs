...
namespace CazamioProject.DBHelpers
{
    public class DBRequestAspNetUsers
    {
        ...
        public class AspNetUsers
        {
            ...
            public static string DeleteFromAspNetUsersNewlyCreatedTenantsAppOccGuarNotAddedToSystem(string emailTenantCreator, string emailTenantOccupant, string emailTenantGuarantor, int marketplaceId)
            {
                string data = null;
                using (SqlConnection db = new(ConnectionDb.GET_CONNECTION_STRING_TO_DB))
                {
                    SqlCommand command = new("DELETE FROM ChatCursors WHERE UserId" +
                               ...
                               " WHERE UserId IN(SELECT Id FROM AspNetUsers WHERE Email = @emailTenantCreator AND MarketplaceId = @marketplaceId)" +
                               " DELETE FROM AspNetUsers" +
                               " WHERE Email = @emailTenantCreator AND MarketplaceId = @marketplaceId" +
                               " DELETE FROM ChatCursors WHERE UserId" +
                               " IN" +
                               " (SELECT UserId FROM Tenants WHERE UserId" +
                               " IN" +
                               " (SELECT Id FROM AspNetUsers WHERE Email = @emailTenantOccupant AND MarketplaceId = @marketplaceId))" +
                               " DELETE FROM Tenants" +
                               " WHERE UserId IN(SELECT Id FROM AspNetUsers WHERE Email = @emailTenantOccupant AND MarketplaceId = @marketplaceId)" +
                               " DELETE FROM AspNetUsers" +
                               " WHERE Email = @emailTenantOccupant AND MarketplaceId = @marketplaceId" +
                               " DELETE FROM ChatCursors WHERE UserId" +
                               " IN" +
                               " (SELECT UserId FROM Tenants WHERE UserId" +
                               " IN" +
                               ...
                               " DELETE FROM Tenants" +
                               " WHERE UserId IN(SELECT Id FROM AspNetUsers WHERE Email = @emailTenantGuarantor AND MarketplaceId = @marketplaceId)" +
                               " DELETE FROM AspNetUsers" +
                               " WHERE Email = @emailTenantGuarantor AND MarketplaceId = @marketplaceId", db);
                    command.Parameters.AddWithValue("@emailTenantCreator", DbType.String).Value = emailTenantCreator;
