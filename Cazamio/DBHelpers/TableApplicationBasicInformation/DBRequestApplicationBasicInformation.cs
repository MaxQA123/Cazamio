...
{
    public class DBRequestApplicationBasicInformation
    {
        ...
        public class ApplicationBasicInformation
        {
            public static string DeleteRecordByApartmentApplicationId(long? apartmentApplicationId, string emailTenant, int marketplaceId)
            {
                ...
                {
                    SqlCommand command = new("DELETE FROM ApplicationBasicInformation WHERE ApartmentApplicationId = @ApartmentApplicationId AND TenantId" +
                               " IN" +
